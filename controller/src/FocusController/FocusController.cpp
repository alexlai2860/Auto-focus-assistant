...
bool FocusController::FocusInit(int64 &t0, int &lens_num)
{
    ...
    if (lens_num > 0)
    {
        ...
        {
            rsProcessFrame(face1, dis1, t0, data);
        }
    }
    else
    {
 ...
 * @param data
 */
void FocusController::rsProcessFrame(Face &face, Dis &dis, int64 &t0, Data &data)
{
    int key;
    TransferData readData, writeData;
    // RsReader reader;
    ...
    while (key != 27)
    {
        ...
        // cout << this->detect_count << endl;
        DIS = Decider(face, dis, __reader->color, d16, detect_count);
        // 计算差值，写入串口，同时进行异常处理
        ...
        // 方案二:使用插值法
        int target_pulse = disInterPolater(DIS);
        cv::putText(__reader->color, cv::format("%d", target_pulse), cv::Point2i(15, 60), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 0), 2);
 ...
 * @param d16
 */
void FocusController::dropProcess(int mode, Dis &dis, cv::Mat &d16)
{
    cout << "mode : " << mode << endl;
    switch (mode)
    {
 ...
 * @return int
 */
int FocusController::Decider(Face &face, Dis &dis, cv::Mat &color, cv::Mat &d16, int &detect_count)
{
    // 复杂的判断过程(待简化)
    int DIS;
    int situation = 0;
